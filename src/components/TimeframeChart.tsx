...
export function isRawTimeframe(tf: string) {
  return ['50ms', '200ms', '1s', '5s', '30s'].includes(tf);
}
...
export default function TimeframeChart({
  ...
  if (isRaw) {
    const prices = rawData.map((d) => d.price).filter((p) => Number.isFinite(p));
    const minP = prices.length ? Math.min(...prices) : 0;
    const maxP = prices.length ? Math.max(...prices) : 1;
    const pad = (maxP - minP) * 0.08 || Math.max(minP * 0.002, 0.000001);

    const chartData = rawData.map((d, i) => ({ ...d, index: i }));
    const label = `Raw Ticks · ${timeframe}`;
    const clampedFrame = Math.min(frame, Math.max(rawData.length - 1, 0));
  ...
  const minCandlesNeeded = timeframe === '5m' || timeframe === '15m' || timeframe === '1h' ? 2 : 1;
  ...
  const padding = rangeP * 0.08 || Math.max(minP * 0.002, 0.000001);
  ...
  const tfLabel = showVolume ? `Line · ${timeframe}` : `Line · ${timeframe}`;
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: mainChartHeight }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: mainTopOffset, right: chartSidePadding, left: chartSidePadding, bottom: 0 }}>
